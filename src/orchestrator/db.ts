...
function createSchema(
  ...
  // Add model column for per-task model override (model router)
  try {
    database.exec(
...
/**
 * Look up a single message by its TG message_id (stored as string in `id`)
 * within a specific chat. Returns full row including the actual transcribed
 * content (e.g. "[Voice: ...transcript...]") so reply-to handlers can
 * surface what was originally said, not just "[voice message]" placeholder.
 */
...
/**
 * Search recent messages in a chat. Used by the MCP `chat_history` tool so
 * the agent can pull older messages on demand (referenced topics, names,
 * URLs that left the recent <messages> window). Filters out bot's own
 * messages by default unless includeBot=true.
 */
export function searchChatMessages(
  ...
  const sql = `SELECT id, content, sender_name, timestamp, is_from_me FROM messages WHERE ${conditions.join(' AND ')} ORDER BY timestamp DESC LIMIT ?`;
  params.push(limit);
...
export function deleteTask(id: string): void {
  // Delete child records first (FK constraint), wrapped in transaction
  const deleteTaskTxn = db.transaction(() => {
