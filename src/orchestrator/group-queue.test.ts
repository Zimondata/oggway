...
describe('GroupQueue', () => {
  ...
  it('drains messages before tasks for same group', async () => {
    ...
    // Messages always drain before tasks (user messages take priority)
    expect(executionOrder[0]).toBe('messages'); // first call
    expect(executionOrder[1]).toBe('messages'); // messages drain before tasks
    // Task runs after pending messages are handled
  });
