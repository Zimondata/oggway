...
/**
 * Collect envKeys declared in installed extension manifests under
 * provides.envKeys. Mirrors getExtensionAllowedDomains().
 *
 * This is the path for prompt-only extensions (manifest.json + skills/, no
 * code) which never call registerExtension(), so getExtensionContainerEnvKeys()
 * collects nothing for them. The runners merge both: containerEnvKeys for
 * code-registered extensions, this for manifest-declared ones.
 *
 * @param extensionsDirOverride optional dir to scan instead of the real
 *   extensions/ dir (tests only). Production callers pass nothing.
 */
export function getExtensionDeclaredEnvKeys(extensionsDirOverride?: string): string[] {
  const extensionsDir = extensionsDirOverride ?? path.join(getCodeRoot(), 'extensions');
  const keys: string[] = [];

  if (!fs.existsSync(extensionsDir)) return keys;
  ...
  for (const entry of entries) {
    ...
    try {
      const raw = JSON.parse(fs.readFileSync(manifestPath, 'utf-8'));
      const { valid, manifest } = validateManifest(raw);
      if (valid && manifest?.provides?.envKeys) {
        keys.push(...manifest.provides.envKeys);
      }
    } catch {
      // Skip invalid manifests
    }
