...
import { getExtensionDeclaredEnvKeys } from '../orchestrator/extension-loader.js';
import {
  CONTAINER_IMAGE,
...
import type { ChannelTurnLog } from '../cost-tracking/index.js';
...
export interface ContainerOutput {
  ...
  durationMs?: number;
  turns?: number;
  /** Number of distinct tool names invoked during the run (for reflection-trigger heuristics). */
  ...
  /**
   * Streaming: this output represents an intermediate (partial) assistant
   * message, not the final SDK result. The host treats it as a hint to
   * edit the in-flight Telegram draft, not to send a fresh message.
   * `partialText` is the full accumulated text up to this point (not a delta).
   */
  partial?: boolean;
  partialText?: string;
}
