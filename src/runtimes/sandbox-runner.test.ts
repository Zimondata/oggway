...
describe('buildSandboxSettings', () => {
  it('puts readonly mounts in allowRead only (nested allowWrite needs no explicit denyWrite)', () => {
    ...
    expect(settings.filesystem.denyWrite).not.toContain('/host/project');
  });
  ...
  it('merges extra allowed write paths into allowWrite', () => {
    const settings = buildSandboxSettings([], [], ['/Users/pine/backups']);
