...
export function isAdminCommand(text: string): boolean {
  return (
    text.startsWith('/') &&
    /^\/(status|logs|restart|revert|push|clear-session|shell|help)\b/i.test(
      text,
    )
  );
}
...
async function cmdStatus(): Promise<string> {
  ...
  if (process.platform === 'darwin') {
    ...
    lines.push(
      line ? `service: ${line.trim()}` : `service: NOT LOADED (${label})`,
    );
  } else {
    const { stdout } = await runCmd(
      'systemctl',
      ['--user', 'is-active', `claudeclaw-${path.basename(STATE_ROOT)}`],
      5000,
    );
  ...
  lines.push(
    `uptime (service PID): ${process.pid}, started ${new Date(Date.now() - Math.round(process.uptime() * 1000)).toISOString()}`,
  );
  ...
  try {
    ...
    const groups = db
      .prepare('SELECT COUNT(*) as n FROM registered_groups')
      .get() as { n: number };
    const tasks = db
      .prepare(
    ...
    const recentRuns = db
      .prepare(
        "SELECT COUNT(*) as n FROM agent_runs WHERE run_at > datetime('now', '-1 day')",
      )
      .get() as { n: number };
    db.close();
    lines.push(
      `groups: ${groups.n}, active tasks: ${tasks.n}, runs last 24h: ${recentRuns.n}`,
    );
  ...
  // Git state
  try {
    const { stdout: branch } = await runCmd(
      'git',
      ['-C', STATE_ROOT, 'rev-parse', '--abbrev-ref', 'HEAD'],
      5000,
    );
    const { stdout: head } = await runCmd(
      'git',
      ['-C', STATE_ROOT, 'log', '-1', '--oneline'],
      5000,
    );
    const { stdout: dirty } = await runCmd(
      'git',
      ['-C', STATE_ROOT, 'status', '--porcelain'],
      5000,
    );
    lines.push(
      `git ${branch.trim()}: ${head.trim()}${dirty.trim() ? ' [dirty]' : ''}`,
    );
  } catch {
    /* not a git repo */
  }
...
async function cmdRestart(): Promise<string> {
  if (process.platform === 'darwin') {
    const { stdout, stderr, code } = await runCmd(
      ...
      [
        'kickstart',
        '-k',
        `gui/${process.getuid?.() || 501}/${getServiceLabel()}`,
      ],
      15000,
    );
    return code === 0
      ? `restart triggered: ${getServiceLabel()}`
      : `restart failed (${code}): ${stderr || stdout}`;
  }
  ...
  return code === 0
    ? `restart triggered: ${unit}`
    : `restart failed (${code}): ${stderr || stdout}`;
}

async function cmdRevert(): Promise<string> {
  const { stdout: head, code: codeHead } = await runCmd(
    'git',
    ['-C', STATE_ROOT, 'log', '-1', '--oneline'],
    5000,
  );
  ...
  const {
    ...
    'git',
    ['-C', STATE_ROOT, 'revert', '--no-edit', 'HEAD'],
    15000,
  );
  if (code !== 0) return `revert failed (${code}):\n${err || out}`;
  const { code: buildCode, stderr: buildErr } = await runCmd(
    'npm',
    ['--prefix', STATE_ROOT, 'run', 'build'],
    120000,
  );
  if (buildCode !== 0)
    return `revert ok but build failed:\n${buildErr.slice(-500)}\n\nrun /restart manually once fixed`;
