...
// ---------------------------------------------------------------------------
export function setupChatApi(app, dbPath) {
  ...
  `);
    ...
    const topicCount = ro.prepare('SELECT COUNT(*) as cnt FROM dashboard_topics').get();
    if (topicCount.cnt === 0) {
        const insert = rw.prepare('INSERT INTO dashboard_topics (id, name, category, icon, system_context, sort_order) VALUES (?, ?, ?, ?, ?, ?)');
        const defaults = [
    ...
    app.get('/api/topics', (_req, res) => {
        try {
            const topics = ro.prepare('SELECT id, name, category, icon, system_context, sort_order, created_at FROM dashboard_topics ORDER BY sort_order, name').all();
            res.json({ topics });
        }
        catch (err) {
    ...
    app.post('/api/topics', (req, res) => {
        try {
            ...
            rw.prepare('INSERT OR REPLACE INTO dashboard_topics (id, name, category, icon, system_context, sort_order) VALUES (?, ?, ?, ?, ?, ?)').run(id, name, category || '', icon || '', system_context || '', sort_order || 0);
            res.json({ ok: true });
        }
        catch (err) {
    ...
    app.post('/api/chat/send', (req, res) => {
        try {
            ...
            if (topicId && topicId !== 'general') {
                const topic = ro.prepare('SELECT system_context, name FROM dashboard_topics WHERE id = ?').get(topicId);
                if (topic?.system_context) {
                    finalContent = `[Topic: ${topic.name}] ${content}`;
                }
            }
            // Insert into ClaudeClaw's messages table
            rw.prepare('INSERT INTO messages (id, chat_jid, sender, sender_name, content, timestamp, is_from_me, is_bot_message) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(msgId, chatJid, 'Btchit', 'Btchit', finalContent, timestamp, 0, 0);
    ...
    app.get('/api/chat/stream', (req, res) => {
        const topicId = req.query.topic || 'general';
        res.writeHead(200, {
        ...
        const client = { res, topicId, lastTs: new Date().toISOString() };
        ...
        for (const client of sseClients) {
            try {
                ...
                if (client.topicId === 'general') {
                    rows = ro.prepare(`SELECT id, sender, sender_name, content, timestamp, is_from_me, is_bot_message
             FROM messages
             WHERE chat_jid = ? AND timestamp > ?
             ORDER BY timestamp ASC LIMIT 50`).all(chatJidGlobal, client.lastTs);
                }
                else {
                    rows = ro.prepare(`SELECT m.id, m.sender, m.sender_name, m.content, m.timestamp, m.is_from_me, m.is_bot_message
             FROM messages m
             INNER JOIN message_topics mt ON mt.message_id = m.id
             WHERE m.chat_jid = ? AND mt.topic_id = ? AND m.timestamp > ?
             ORDER BY m.timestamp ASC LIMIT 50`).all(chatJidGlobal, client.topicId, client.lastTs);
                }
                for (const msg of rows) {
                    try {
                        client.res.write(`data: ${JSON.stringify({ type: 'message', data: msg })}\n\n`);
                    }
                    catch { /* dead */ }
                    if (msg.timestamp > client.lastTs)
                        client.lastTs = msg.timestamp;
                }
            }
            catch (err) {
    ...
    app.get('/api/bot-status', (_req, res) => {
        try {
            ...
            if (!alive) {
                try {
                    const recentActivity = ro.prepare(`SELECT 1 FROM messages
            ...
            if (!alive) {
                try {
                    const recentRun = ro.prepare("SELECT 1 FROM agent_runs WHERE run_at >= datetime('now', '-10 minutes') LIMIT 1").get();
                    if (recentRun)
                        alive = true;
                }
                catch { }
            ...
            // Last agent run
            const lastRun = ro.prepare('SELECT run_at, status, duration_ms, turns, model FROM agent_runs ORDER BY run_at DESC LIMIT 1').get();
            // Queue depth (messages not yet processed - approximation)
            const pendingMessages = ro.prepare(`SELECT COUNT(*) as cnt FROM messages
         WHERE is_bot_message = 0 AND is_from_me = 0
         AND timestamp > COALESCE((SELECT MAX(run_at) FROM agent_runs), '2000-01-01')`).get();
            // Active session
            const session = ro.prepare("SELECT session_id FROM sessions WHERE group_folder = 'personal'").get();
            res.json({
                alive,
                pid,
