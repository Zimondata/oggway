...
function findDb() {
    ...
    throw new Error(`Cannot find store/messages.db. Run from ClaudeClaw root or set DB_PATH env var.\nSearched: ${candidates.join(', ')}`);
}
...
function tokenRates(model) {
    const m = (model || '').toLowerCase();
    if (m.includes('opus'))
        return { inputRate: 15 / 1e6, outputRate: 75 / 1e6 };
    if (m.includes('haiku'))
        return { inputRate: 0.25 / 1e6, outputRate: 1.25 / 1e6 };
    // default to sonnet
...
app.get('/api/costs', (req, res) => {
    try {
        ...
        const hitRate = totalInputPlusCacheRead > 0
        ...
        try {
            const msgRow = db
                .prepare(`SELECT COUNT(*) as cnt FROM messages WHERE timestamp >= ? AND is_from_me = 0`)
                .get(since);
...
app.get('/api/groups', (_req, res) => {
    try {
        ...
        try {
            registered = db
                .prepare(`SELECT folder, name, runtime, is_main as isMain,
        ...
        const groupList = folders.map((folder) => {
            ...
            if (meta?.agentConfig) {
                try {
                    agentConfig = JSON.parse(meta.agentConfig);
                }
                catch { }
            }
            return {
...
app.get('/api/tasks', (_req, res) => {
    try {
        ...
        try {
            scheduled = db
                .prepare(`SELECT id, group_folder as groupFolder, prompt, schedule_type as scheduleType,
        ...
        const recent = db
            .prepare(`SELECT rowid as id, group_folder as groupFolder, trigger_type as triggerType,
...
app.get('/api/memory/tree', (req, res) => {
    try {
        const group = req.query.group || 'personal';
        ...
        for (const name of readdirSync(groupRoot).sort()) {
            if (name === 'CLAUDE.md' || name === 'memory' || name.startsWith('.'))
                continue;
...
app.get('/api/logs', (req, res) => {
    try {
        const fileName = req.query.file || 'claudeclaw.log';
