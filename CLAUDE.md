...
## Browser daemon (optional)

Agents get a real Chromium browser through `scripts/browser-daemon.sh`. The daemon runs OUTSIDE the sandbox (full network access); the agent connects to it through the `claudeclaw-browser` MCP server over CDP on `localhost:9222`. Login state survives restarts via `data/browser-profile/`.

```bash
./scripts/browser-daemon.sh start    # Launch Chromium with CDP
./scripts/browser-daemon.sh status   # Check if running
./scripts/browser-daemon.sh stop
```

**First-time setup** (per site): start daemon, ask the agent to open the site, complete login (including 2FA) in the visible Chromium window. Cookies persist; later runs are non-interactive.

**Tools available to the agent:** `browser_navigate`, `browser_url`, `browser_text`, `browser_snapshot`, `browser_click`, `browser_fill`, `browser_press`, `browser_wait`, `browser_new_tab`, `browser_list_tabs`, `browser_select_tab`, `browser_close_tab`. Configurable via `CLAUDECLAW_BROWSER_CDP_URL` env var.

If the daemon isn't running, browser tools return a clear error pointing to the start command — they don't fail the agent run.
